...
inline bool MSHWriter::WriteMeshData(FbxNode* pMeshNode)
{
    ...
    if(pMesh){
        ...
        if(!pMesh->IsTriangleMesh()){
            ...
            wcscpy(errorMSG, LANG_MSG_NOT_TRIANGLE);
            MultiByteToWideChar(CP_UTF8, NULL, pMesh->GetName(), -1, tName, 64);
            wcscat(errorMSG, tName);
            wcscat(errorMSG, LANG_MSG_QUESTION);
            if(MessageBox(0, errorMSG, LANG_MSG_QUESTION, MB_ICONWARNING | MB_OKCANCEL) == IDCANCEL)
                return false;
        }
        ...
        FbxLayerElementArrayTemplate<FbxVector4>* originNormalData = nullptr;  //法线数据
        FbxLayerElementArrayTemplate<FbxVector2>* originUVData = nullptr;     //UV数据
        FbxLayerElementArrayTemplate<int>* originUVIndex = nullptr;
        FbxGeometry* pMeshAttribute;
